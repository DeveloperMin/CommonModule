...
@interface MyCirclePhotoView ()

@property (nonatomic, strong) NSArray *imageViewsArray;

@end

@implementation MyCirclePhotoView
...
- (void)setImgModels:(NSArray *)imgModels {
    ...
    [_imgModels enumerateObjectsUsingBlock:^(TXBYCircleImg *imageModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        imageView.hidden = NO;
        [imageView sd_setImageWithURL:[NSURL URLWithString:imageModel.url] placeholderImage:[UIImage imageNamed:@"avatar_default"]];
        imageView.frame = CGRectMake(columnIndex * (itemW + margin), rowIndex * (itemH + margin), itemW, itemH);
    }];
    CGFloat w = [UIScreen mainScreen].bounds.size.width - 74;
...
- (CGFloat)itemWidthForPicPathArray:(NSArray *)array {
    if (array.count == 1) {
        ...
        if (wh <= 0.75) {
            return [UIScreen mainScreen].bounds.size.width - 74;
        ...
        if (array.count < 3) {
            return (TXBYApplicationW - 74 - (array.count - 1) * 2) / array.count;
        } else {
            CGFloat w = (TXBYApplicationW - 78) / 3;
            return w;
        }
    }
}
