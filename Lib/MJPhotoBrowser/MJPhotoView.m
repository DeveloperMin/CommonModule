...
- (void)hide
{
    ...
    UIImage *srcImage = _photo.srcImageView.image;
    _photo.srcImageView.image = nil;
    [UIView animateWithDuration:duration + 0.1 animations:^{
        CGRect frame = _imageView.frame;
        CGRect srcImageViewFrame = [_photo.srcImageView convertRect:_photo.srcImageView.bounds toView:nil];
        // 防止图片跳动
        if (_photo.srcImageView.contentMode == UIViewContentModeScaleAspectFill && fabs(srcImageViewFrame.size.width/srcImageViewFrame.size.height - frame.size.width/frame.size.height) > 1e-6) {
            if (frame.size.width <= frame.size.height) {
                CGFloat originalSrcImageHeight = srcImageViewFrame.size.height;
                srcImageViewFrame.size.height = originalSrcImageHeight * frame.size.height / frame.size.width;
                srcImageViewFrame.origin.y -= (srcImageViewFrame.size.height -  originalSrcImageHeight) * 0.5;
            } else {
                CGFloat originalSrcImageWidth = srcImageViewFrame.size.height;
                srcImageViewFrame.size.width = originalSrcImageWidth * frame.size.width / frame.size.height;
                srcImageViewFrame.origin.x -= (srcImageViewFrame.size.width -  originalSrcImageWidth) * 0.5;
            }
        }
        
        if (frame.size.width != 0 && frame.size.height != 0 ) {
            _imageView.frame = srcImageViewFrame;
        }
        ...
        _photo.srcImageView.image = srcImage;
        // 通知代理
